...
def test_config_help():
    ...
    assert result.exit_code == 0
    assert "Saves your configurations to ~/.config/savit/config.toml" in result.output
...
def test_stop_help():
    ...
    assert result.exit_code == 0
    assert "Stop saving your commands and writes them to a file" in result.output
